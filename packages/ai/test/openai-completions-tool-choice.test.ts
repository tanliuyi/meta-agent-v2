...
describe("openai-completions tool_choice", () => {
	...
	it("stores z.ai tool_stream support in model compat metadata", () => {
		...
		expect(getModel("opencode", "glm-5.1")?.compat?.zaiToolStream).toBeUndefined();
	});
	...
	it("omits tool_stream for unsupported models", async () => {
		const model = getModel("opencode", "glm-5.1")!;
		const tools: Tool[] = [
			{
				name: "ping",
				description: "Ping tool",
	...
	it("respects explicit tool_stream compat override", async () => {
		const baseModel = getModel("opencode", "glm-5.1")!;
		const model = {
			...baseModel,
			compat: {
				...baseModel.compat,
