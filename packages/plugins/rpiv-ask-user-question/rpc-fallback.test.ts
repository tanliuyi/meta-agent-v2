import { createMockCtx, createMockPi, mockStdout } from "./test-fixtures";
import { describe, expect, it, vi } from "vitest";
import { BEL, registerAskUserQuestionTool } from "./ask-user-question";
import { hasDialogUI, runRpcQuestionnaire } from "./rpc-fallback";
...
describe("runRpcQuestionnaire cancellation", () => {
	it("forwards the abort signal to the active native dialog", async () => {
		const controller = new AbortController();
		const select = vi.fn(
			(_title: string, _options: string[], dialogOptions?: { signal?: AbortSignal }) =>
				new Promise<string | undefined>((resolve) => {
					dialogOptions?.signal?.addEventListener("abort", () => resolve(undefined), { once: true });
				}),
		);
		const pending = runRpcQuestionnaire(
			{ select, input: vi.fn(async () => "") },
			SINGLE,
			controller.signal,
		);
		...
		expect(select).toHaveBeenCalledWith(expect.any(String), expect.any(Array), { signal: controller.signal });
	});
});
