...
import type { QuestionnaireResult } from "./tool/types";
...
async function registerFresh() {
	const { registerAskUserQuestionTool } = await import("./ask-user-question");
...
describe("ask_user_question — session-start pre-warm (#107)", () => {
	it("schedules a background import for hosts without structured questionnaire support", async () => {
		vi.useFakeTimers();
		const factory = vi.fn(() => ({ QuestionnaireSession: class {} }));
		vi.doMock(SESSION_SPECIFIER, factory);
		const { registerAskUserQuestionTool, PREWARM_DELAY_MS } = await import("./ask-user-question");
		const { pi, captured } = createMockPi();
		registerAskUserQuestionTool(pi);
		...
		for (const handler of captured.events.get("session_start") ?? []) {
			await handler({}, createMockCtx({ hasUI: true }));
		}
