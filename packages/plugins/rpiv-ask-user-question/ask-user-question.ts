...
import { hasQuestionnaireUI } from "./desktop-questionnaire";
import {
...
import { type DialogUI, hasDialogUI, runRpcQuestionnaire } from "./rpc-fallback";
import { displayLabel, t } from "./state/i18n-bridge";
import { sentinelsToAppend } from "./state/row-intent";
import { normalizeQuestionParams } from "./tool/normalize-params";
...
type SessionModule = typeof import("./state/questionnaire-session");

type SessionRef = { current: import("./state/questionnaire-session").QuestionnaireSession | null };
...
export async function loadQuestionnaireSession(): Promise<SessionLoad> {
	...
	try {
		mod = await import("./state/questionnaire-session");
	} catch (e) {
...
function makeSessionFactory(config: {
	...
	return (
		tui: TUI,
		theme: Theme,
		keybindings: import("./state/questionnaire-session").QuestionnaireSessionConfig["keybindings"],
		done: (result: QuestionnaireResult) => void,
	): import("./state/questionnaire-session").QuestionnaireSessionComponent => {
...
export function registerAskUserQuestionTool(pi: ExtensionAPI): void {
	...
	pi.registerTool({
		...
		async execute(_toolCallId, params, signal, _onUpdate, ctx) {
			...
			if (hasQuestionnaireUI(ctx.ui)) {
				emitAskUserBlockedEvent(pi, true);
				try {
					return buildQuestionnaireResponse(await ctx.ui.questionnaire(typed, { signal }), typed);
				} finally {
					emitAskUserBlockedEvent(pi, false);
				}
			}
			...
			// RPC and Desktop hosts expose native select/input dialogs but do not expose
			// the TUI custom renderer. Route both through the host-native walker so the
			// Desktop sidecar never imports the pi-tui questionnaire graph.
			const supportsCustomUi = typeof (ctx.ui as { custom?: unknown }).custom === "function";
			if (hasDialogUI(ctx.ui) && ((ctx as { mode?: string }).mode === "rpc" || !supportsCustomUi)) {
	...
	pi.on("session_start", (_event, ctx) => {
		if (!hasQuestionnaireUI(ctx.ui)) prewarmSessionGraph();
	});
}
