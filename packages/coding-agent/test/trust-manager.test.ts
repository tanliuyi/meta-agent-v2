...
describe("ProjectTrustStore", () => {
	...
	it("detects trust-requiring project resources", () => {
		...
		try {
			mkdirSync(join(tempDir, ".pi-desk", "agent"), { recursive: true });
			...
			writeFileSync(join(tempDir, ".pi-desk", "settings.json"), "{}");
			expect(hasTrustRequiringProjectResources(tempDir)).toBe(true);
			rmSync(join(tempDir, ".pi-desk", "settings.json"), { force: true });

			mkdirSync(join(cwd, ".pi-desk"), { recursive: true });
			writeFileSync(join(cwd, ".pi-desk", "settings.json"), "{}");
			expect(hasTrustRequiringProjectResources(cwd)).toBe(true);

			rmSync(join(cwd, ".pi-desk"), { recursive: true, force: true });
