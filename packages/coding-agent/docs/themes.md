...
- Built-in: `dark`, `light`
- Global: `~/.pi-desk/agent/themes/*.json`
- Project: `.pi-desk/themes/*.json` (only after the project is trusted)
...
```bash
mkdir -p ~/.pi-desk/agent/themes
vim ~/.pi-desk/agent/themes/my-theme.json
```
