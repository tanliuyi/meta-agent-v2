...
Uninstalling pi leaves settings, credentials, sessions, and installed pi packages in `~/.pi-desk/agent/`.
...
You can also run `/login` and select an API-key provider to store the key in `~/.pi-desk/agent/auth.json`.
