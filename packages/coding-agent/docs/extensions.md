...
> **Placement for /reload:** Put extensions in `~/.pi-desk/agent/extensions/` (global) or `.pi-desk/extensions/` (project-local) for auto-discovery. Use `pi -e ./path.ts` only for quick tests. Extensions in auto-discovered locations can be hot-reloaded with `/reload`.
...
Create `~/.pi-desk/agent/extensions/my-extension.ts`:
...
Extensions are auto-discovered from trusted locations. Project-local `.pi-desk/extensions` entries load only after the project is trusted.

| Location | Scope |
|----------|-------|
| `~/.pi-desk/agent/extensions/*.ts` | Global (all projects) |
| `~/.pi-desk/agent/extensions/*/index.ts` | Global (subdirectory) |
| `.pi-desk/extensions/*.ts` | Project-local |
| `.pi-desk/extensions/*/index.ts` | Project-local (subdirectory) |
...
Fired before pi decides whether to trust a project with dynamic configs (`.pi-desk` or `.agents/skills`). It runs during startup and when session replacement (for example `/resume`) enters a cwd whose trust has not been resolved in the current process. Only user/global extensions and CLI `-e` extensions participate; project-local extensions are not loaded until after trust is resolved.
...
Use `CONFIG_DIR_NAME` instead of hardcoding `.pi-desk` when constructing project-local config paths. Rebranded distributions can use a different config directory name.
