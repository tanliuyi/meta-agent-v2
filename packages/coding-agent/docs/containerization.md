...
```bash
cp -R packages/coding-agent/examples/extensions/gondolin ~/.pi-desk/agent/extensions/gondolin
cd ~/.pi-desk/agent/extensions/gondolin
...
```bash
cd /path/to/project
pi -e ~/.pi-desk/agent/extensions/gondolin
```
...
```bash
docker build -t pi-sandbox -f Dockerfile.pi-desk .

docker run --rm -it \
  -e ANTHROPIC_API_KEY \
  -v "$PWD:/workspace" \
  -v pi-agent-home:/root/.pi-desk/agent \
  pi-sandbox
```

The `-v "$PWD:/workspace"` mounts your current directory into the container at /workspace such that reads and writes in `/workspace` inside Docker directly affect your host files, like in the Gondolin example.

Use a named volume for `/root/.pi-desk/agent` if you want container-local settings and sessions. Mounting your host `~/.pi-desk/agent` exposes host auth and session files to the container.
