...
const { session } = await createAgentSession({
  ...
  agentDir: "~/.pi-desk/agent", // default (expands ~)
});
```

`cwd` is used by `DefaultResourceLoader` for:
- Project extensions (`.pi-desk/extensions/`)
- Project skills:
  - `.pi-desk/skills/`
  - `.agents/skills/` in `cwd` and ancestor directories (up to git repo root, or filesystem root when not in a repo)
- Project prompts (`.pi-desk/prompts/`)
...
- Global skills:
  - `skills/` under `agentDir` (for example `~/.pi-desk/agent/skills/`)
  - `~/.agents/skills/`
...
// Default: uses ~/.pi-desk/agent/auth.json and ~/.pi-desk/agent/models.json
const modelRuntime = await ModelRuntime.create();
...
Extensions are loaded by the `ResourceLoader`. `DefaultResourceLoader` discovers extensions from `~/.pi-desk/agent/extensions/`, `.pi-desk/extensions/`, and settings.json extension sources.
...
1. Global: `~/.pi-desk/agent/settings.json`
2. Project: `<cwd>/.pi-desk/settings.json`
