...
By default, `install` and `remove` write to user settings (`~/.pi-desk/agent/settings.json`). Use `-l` to write to project settings (`.pi-desk/settings.json`) instead. Project settings can be shared with your team, and pi installs any missing packages automatically on startup after the project is trusted.
...
- User installs go under `~/.pi-desk/agent/npm/`.
- Project installs go under `.pi-desk/npm/`.
...
- Cloned to `~/.pi-desk/agent/git/<host>/<path>` (global) or `.pi-desk/git/<host>/<path>` (project).
...
Use `pi config` to enable or disable extensions, skills, prompt templates, and themes from installed packages and local directories. `pi config` starts in global settings (`~/.pi-desk/agent/settings.json`); press Tab to switch between global and project-local modes. Use `pi config -l` to start in project overrides (`.pi-desk/settings.json`) with inherited global resources dimmed.
