...
| `.pi-desk/settings.json` | Project (current directory) |
...
`npmCommand` is used for all npm package-manager operations, including installs, uninstalls, and dependency installs inside git packages. User-scoped npm packages install under `~/.pi-desk/agent/npm/`; project-scoped npm packages install under `.pi-desk/npm/`. Use argv-style entries exactly as the process should be launched. When `npmCommand` is configured, git package dependency installs use plain `install` to avoid npm-specific flags in wrappers or alternate package managers.
...
Paths in `~/.pi-desk/agent/settings.json` resolve relative to `~/.pi-desk/agent`. Paths in `.pi-desk/settings.json` resolve relative to `.pi-desk`. Absolute paths and `~` are supported.
...
Project settings (`.pi-desk/settings.json`) override global settings. Nested objects are merged:

```json
// ~/.pi-desk/agent/settings.json (global)
{
  "theme": "dark",
  "compaction": { "enabled": true, "reserveTokens": 16384 }
}

// .pi-desk/settings.json (project)
{
