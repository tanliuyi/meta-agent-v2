...
**Custom providers & models:** Add providers via `~/.pi-desk/agent/models.json` if they speak a supported API (OpenAI, Anthropic, Google). For custom APIs or OAuth, use extensions. See [docs/models.md](docs/models.md) and [docs/custom-provider.md](docs/custom-provider.md).
...
See `/hotkeys` for the full list. Customize via `~/.pi-desk/agent/keybindings.json`. See [docs/keybindings.md](docs/keybindings.md).
...
Sessions auto-save to `~/.pi-desk/agent/sessions/` organized by working directory.
...
| Location | Scope |
|----------|-------|
| `~/.pi-desk/agent/settings.json` | Global (all projects) |
| `.pi-desk/settings.json` | Project (overrides global) |

See [docs/settings.md](docs/settings.md) for all options.

### Project Trust

On interactive startup, pi asks before trusting a project folder that contains project-local settings, resources, or project `.agents/skills` and has no saved decision for the folder or a parent folder in `~/.pi-desk/agent/trust.json`. Trusting a project allows pi to load `.pi-desk/settings.json` and `.pi-desk` resources, install missing project packages, and execute project extensions.
...
If no extension or saved decision applies, `defaultProjectTrust` controls the fallback behavior. Set it to `"ask"`, `"always"`, or `"never"` in `~/.pi-desk/agent/settings.json`, or change it with `/settings`.

`pi config` and package commands use the same project trust flow, except `pi update` never prompts. Pass `--approve` to trust project-local settings for one command or `--no-approve` to ignore them.

Use `/trust` in interactive mode to save a project trust decision for future sessions, including trust for the immediate parent folder. It writes `~/.pi-desk/agent/trust.json` only; the current session is not reloaded, so restart pi for changes to take effect.
...
- `~/.pi-desk/agent/AGENTS.md` (global)
...
Replace the default system prompt with `.pi-desk/SYSTEM.md` (project) or `~/.pi-desk/agent/SYSTEM.md` (global). Append without replacing via `APPEND_SYSTEM.md`.
...
```markdown
<!-- ~/.pi-desk/agent/prompts/review.md -->
Review this code for bugs, security issues, and performance problems.
Focus on: {{focus}}
```

Place in `~/.pi-desk/agent/prompts/`, `.pi-desk/prompts/`, or a [pi package](#pi-packages) to share with others. See [docs/prompt-templates.md](docs/prompt-templates.md).
...
```markdown
<!-- ~/.pi-desk/agent/skills/my-skill/SKILL.md -->
# My Skill
...
Place in `~/.pi-desk/agent/skills/`, `~/.agents/skills/`, `.pi-desk/skills/`, or `.agents/skills/` (from `cwd` up through parent directories) or a [pi package](#pi-packages) to share with others. See [docs/skills.md](docs/skills.md).
...
Place in `~/.pi-desk/agent/extensions/`, `.pi-desk/extensions/`, or a [pi package](#pi-packages) to share with others. See [docs/extensions.md](docs/extensions.md) and [examples/extensions/](examples/extensions/).

### Themes

Built-in: `dark`, `light`. Themes hot-reload: modify the active theme file and pi immediately applies changes.

Place in `~/.pi-desk/agent/themes/`, `.pi-desk/themes/`, or a [pi package](#pi-packages) to share with others. See [docs/themes.md](docs/themes.md).
...
| `PI_CODING_AGENT_DIR` | Override config directory (default: `~/.pi-desk/agent`) |
