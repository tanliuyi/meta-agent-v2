...
describe("plugin configuration service", () => {
  ...
  it("applies defaults, encrypts secrets, and returns plaintext only to the runtime", async () => {
    const initial = await service.getConfig("example.configurable", schema);
    ...
    await expect(service.getRuntimeConfiguration("example.configurable", schema)).rejects.toThrow(
    ...
    await expect(service.getRuntimeConfiguration("example.configurable", schema)).resolves.toEqual({
