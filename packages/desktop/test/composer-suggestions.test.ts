...
describe("ComposerSuggestions", () => {
  ...
  it("命令保留协议名称，仅移除技能协议前缀", () => {
    ...
    expect(slashCommandDisplayName({ name: "memory-interview", source: "extension" })).toBe("memory-interview");
  ...
  it("支持多关键词、来源名称和说明检索", () => {
    ...
    expect(commandSuggestions(commands, "trigger cleanup").map(({ label }) => label)).toEqual(["/memory-consolidate"]);
  ...
  it("使用协议名称和描述检索、展示并插入命令", () => {
    ...
    expect(commandSuggestions(commands, "user profile")).toEqual([
      {
        id: "extension:memory-interview",
        label: "/memory-interview",
        detail: "Answer questions to pre-fill your user profile",
        type: "command",
  ...
  it("保留 Hermes Memory 命令的原始名称", () => {
    ...
    expect(searchSlashCommands(commands, "记忆")).toEqual([]);
  });
