...
export class ThreadWorkerRegistry {
  ...
  async cancel(projectId: string, threadId: string): Promise<void> {
    await this.use(projectId, threadId, (record) => record.client.request({ type: "cancel" }));
  }
  ...
  async remove(projectId: string, threadId: string, policy: SessionRemovePolicy): Promise<SessionRemoveResult> {
    ...
    return this.removeTree(projectId, threadId, policy, catalog, descendantIds);
  }
  ...
  async promote(projectId: string, threadId: string): Promise<SessionRemoveResult> {
    return this.promoteThread(projectId, threadId);
  }
