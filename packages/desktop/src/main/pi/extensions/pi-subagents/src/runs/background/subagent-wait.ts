...
export function asyncWaitStatusItems(
  runs: readonly AsyncRunSummary[],
): Array<{ agent: string; status: string; currentTool?: string; currentPath?: string }> {
  return runs.flatMap((run) => {
    const active = run.steps.filter((step) => step.status === "running" || step.status === "pending");
    if (active.length === 0) return run.state === "queued" || run.state === "running" ? [{ agent: run.id, status: run.state }] : [];
    return active.map((step): { agent: string; status: string; currentTool?: string; currentPath?: string } => ({
      agent: step.agent,
      status: step.status,
