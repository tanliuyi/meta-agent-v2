...
Manifest-backed development entries are the preferred form when the plugin declares a stable identity, capabilities, or configuration. The entry and manifest path must remain inside the selected directory; symlink entries are rejected.
...
- Confirm every configuration default and required field reaches `pi.getConfig()` as expected.
