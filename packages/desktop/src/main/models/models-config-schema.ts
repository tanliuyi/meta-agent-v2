/**
 * Desktop-owned models.json schema, types, and diagnostics.
 *
 * The structural schema mirrors v0.82.1 coding-agent model-config.ts. Desktop
...
/**
 * Parse a JSONC source string into a validated ModelsConfig.
 * Accepts trailing commas and inline comments via jsonc-parser.
 */
...
/**
 * Validate an already-parsed value against the models.json schema.
 */
export function validateModelsConfigValue(value: unknown, path = "models.json"): ModelsConfigValidationResult {
  ...
  return diagnostics.length > 0 ? { ok: false, diagnostics } : { ok: true, value: config, diagnostics };
}
...
function validateModelsConfigSemantics(config: ModelsConfig): ModelsConfigDiagnostic[] {
  ...
  for (const [providerName, providerConfig] of Object.entries(config.providers)) {
    ...
    const hasProviderOverlay = Boolean(providerConfig.baseUrl || providerConfig.headers || providerConfig.compat);

    if (providerConfig.oauth && !providerConfig.baseUrl) {
      diagnostics.push(semanticDiagnostic([...providerPath, "baseUrl"], '"baseUrl" is required when "oauth" is set.'));
    }

    if (models.length === 0 && !providerConfig.oauth && !isBuiltIn && !hasProviderOverlay && !hasModelOverrides) {
      diagnostics.push(
        semanticDiagnostic(providerPath, 'must specify "baseUrl", "headers", "compat", "modelOverrides", or "models".'),
      );
    } else if (models.length > 0 && !isBuiltIn && !providerConfig.baseUrl) {
      diagnostics.push(
...
function validateCost(
  ...
  for (let index = 0; index < (cost.tiers?.length ?? 0); index += 1) {
    ...
    if (tier.inputTokensAbove < 0) {
      diagnostics.push(semanticDiagnostic([...path, "tiers", index, "inputTokensAbove"], "must be zero or greater."));
    }
    if (seenThresholds.has(tier.inputTokensAbove)) {
      diagnostics.push(semanticDiagnostic([...path, "tiers", index, "inputTokensAbove"], "duplicate tier threshold."));
    }
...
/** Human-readable formatting of diagnostics for log/error messages. */
