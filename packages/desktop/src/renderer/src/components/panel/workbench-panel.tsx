...
export function WorkbenchPanel() {
	...
	return <OpenWorkbenchPanel width={workbench.panelWidth} onWidthChange={(panelWidth) => updateWorkbench({ panelWidth })} />;
}
	...
	return (
		<aside className="workbench-panel" style={{ width: resize.size }}>
			...
			<header className="panel-tabs">
				...
				<Button variant="ghost" size="icon" aria-label="关闭 Panel" onClick={() => updateWorkbench({ panelOpen: false })}>
...
function TaskPanel() {
	...
	return (
		<div className="task-panel">
			<h3>会话状态</h3>
			<dl>
				<div><dt>运行</dt><dd>{snapshot.running ? "进行中" : "空闲"}</dd></div>
				<div><dt>上下文</dt><dd>{snapshot.context?.percent === null || snapshot.context?.percent === undefined ? "--" : `${snapshot.context.percent.toFixed(1)}%`}</dd></div>
				<div><dt>队列</dt><dd>{snapshot.queue.steering.length + snapshot.queue.followUp.length}</dd></div>
				<div><dt>压缩</dt><dd>{snapshot.compacting ? "进行中" : "空闲"}</dd></div>
			</dl>
			{Object.keys(snapshot.extensionUi.statuses).length > 0 ? (
				<>
					<h3>扩展状态</h3>
					<ul>{Object.entries(snapshot.extensionUi.statuses).map(([key, value]) => <li key={key}><span>{key}</span><strong>{value}</strong></li>)}</ul>
				</>
			) : null}
		</div>
	);
