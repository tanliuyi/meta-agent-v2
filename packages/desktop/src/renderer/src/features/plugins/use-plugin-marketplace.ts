...
import type { ApplyDesktopExtensionSetResult } from "../../../../shared/desktop-extension-contracts.ts";
import type {
...
export interface PluginMarketplaceController {
  ...
  install(plugin: MarketplacePluginSummary, applyToCurrentSession: boolean): Promise<void>;
  update(plugin: MarketplacePluginSummary, applyToCurrentSession: boolean): Promise<void>;
  uninstall(pluginId: string, applyToCurrentSession: boolean): Promise<void>;
}

export function usePluginMarketplace(returnSession?: {
  projectId: string;
  threadId: string;
  ...
  const returnProjectId = returnSession?.projectId;
  const returnThreadId = returnSession?.threadId;
  ...
  const install = useCallback(
    async (plugin: MarketplacePluginSummary, applyToCurrentSession: boolean) => {
      ...
      try {
        const result = await window.desktop.marketplace.installPlugin({
          ...
          ...applyTarget(applyToCurrentSession, returnProjectId, returnThreadId),
        });
        ...
        if (result.status === "conflict") setError("插件安装状态已变化，请重试");
          ...
          setNotice("插件已在本机安装，已同步最新状态");
        } else if (result.status === "installed" && result.recoveryPending) {
          setNotice("插件已提交，剩余文件状态将在启动恢复中完成");
        } else if (result.status === "installed") {
          setNotice(applicationNotice(result.application, result.applicationError, "插件已安装"));
        }
      } catch (reason) {
    ...
    [installed, installingId, returnProjectId, returnThreadId, updatingId, uninstallingId],
  );
    ...
    async (plugin: MarketplacePluginSummary, applyToCurrentSession: boolean) => {
      ...
      try {
        ...
        if (result.status === "conflict") setError("插件安装状态已变化，请重试");
          ...
          setNotice("插件已不在本机，无法更新，已同步最新状态");
        } else if (result.status === "same-version") {
          setNotice("插件已是该版本，无需更新");
        } else if (result.status === "updated" && result.recoveryPending) {
          setNotice("更新已提交，剩余文件状态将在启动恢复中完成");
        } else if (result.status === "updated") {
          setNotice(applicationNotice(result.application, result.applicationError, "插件已更新"));
        }
      } catch (reason) {
        if (mounted.current) setError(marketplaceErrorMessage(reason));
      } finally {
  ...
  const uninstall = useCallback(
    async (pluginId: string, applyToCurrentSession: boolean) => {
...
function applyTarget(enabled: boolean, projectId: string | undefined, threadId: string | undefined) {
  return enabled && projectId && threadId
    ? { applyToCurrentSession: { projectId, threadId, abortRunning: true as const } }
    : ({} as const);
}

function applicationNotice(
  application: ApplyDesktopExtensionSetResult | undefined,
  applicationError: string | undefined,
  completedMessage: string,
): string {
  if (applicationError) {
    return `${completedMessage}，但应用到当前会话失败：${applicationError}；当前会话尚未重新加载`;
  }
  if (!application) return `${completedMessage}，新会话将使用最新扩展；当前会话尚未重新加载`;
  if (application.status === "applied") return `${completedMessage}并已应用到当前会话`;
  if (application.status === "unchanged") return `${completedMessage}，当前会话已使用该扩展集合`;
  return `当前会话启动失败，插件变更已回滚：${application.error ?? "未知错误"}`;
}
