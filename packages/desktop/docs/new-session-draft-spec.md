...
- `/` command suggestions 展示受控 extension source policy 与资源加载阶段可发现的 extension、prompt 和 skill 命令；
...
应用 cold start 固定进入 `/new`，即使 active Project 已有 regular session 也不自动恢复；已有 session 只通过 Sidebar 显式导航或完整 deep link 打开。
...
- 打开 committed thread 时只卸载 draft UI subtree；窗口级 draft runtime 继续保留 Composer、附件及 Project/model/thinking 配置；
- 从 committed thread 或 settings 返回 `/new` 时复用同一份 draft runtime，不显示丢弃确认；
- 只有首个 prompt 建立真实 session 后才清理 draft；
...
- `components/layout/sidebar.tsx`
  ...
  - 切换 session 时只卸载 draft UI，不触发丢弃确认；窗口级 draft runtime 保留 Composer 和配置。
...
- 离开 `/new` 后卸载 draft UI，不保留 DOM 或 component-local state；
- New 在 draft 中幂等；
...
- draft config 包含同一 `ResolvedExtensionSet` 中的受控 extension、prompt 和 skill commands，并返回 extension-set generation；
...
- 非空 draft 切换 session 或进入 settings 时不显示确认，返回 `/new` 后恢复完整 Composer 和配置；
...
- draft config 通过 Pi session services 发现受控 extension、prompt 和 skill commands，Composer 在 materialize 前即可补全静态命令；
- `sessions.create()` 接受显式 `SessionCreateInput` 和 draft extension-set generation，在 `createAgentSession()` 构造阶段原子应用 model/thinking；stale generation 在创建 writer/JSONL 前失败；
