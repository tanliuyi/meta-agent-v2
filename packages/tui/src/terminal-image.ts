...
function shortenImagePath(filename: string): string {
	...
	const filenameResolved = resolve(filename);
	if (filenameResolved === homeResolved) return "~";
	const prefix = `${homeResolved}${sep}`;
	if (filenameResolved.startsWith(prefix)) {
		return `~/${filenameResolved.slice(prefix.length).replaceAll(sep, "/")}`;
	}
	return filename;
}
