...
export function createPublishControllers(runtime: MarketplaceHttpRuntime): Type<unknown>[] {
	class PublishController {
		...
		async state(pluginId: string, authorization: string | undefined): Promise<{ plugin: PublishPluginState }> {
			...
			return { plugin: await mapStoreErrors(() => runtime.store.publishState(pluginId)) };
		}
		...
		async upsertPlugin(
			...
			return { plugin: await mapStoreErrors(() => runtime.store.upsertPlugin(pluginId, request)) };
		}
		...
		async createVersion(
			...
			return {
				pluginId,
				version: await mapStoreErrors(() => runtime.store.createDraftVersion(pluginId, request)),
			};
		}
		...
		async uploadArtifact(
			...
			validatePluginId(pluginId);
			await requirePluginMember(runtime, authorization, pluginId);
			const context = await mapStoreErrors(() => runtime.store.getUploadContext(pluginId, version, artifactId));
		...
		async publish(
			...
			await requirePluginMember(runtime, authorization, pluginId);
			await mapStoreErrors(() =>
				runtime.store.publishVersion(pluginId, version, (artifacts) => {
		...
		async deprecate(
			...
			validatePluginId(pluginId);
			await requirePluginMember(runtime, authorization, pluginId);
			await mapStoreErrors(() => runtime.store.deprecateVersion(pluginId, version));
		...
		async deleteDraft(pluginId: string, version: string, authorization: string | undefined): Promise<void> {
			validatePluginId(pluginId);
			await requirePluginMember(runtime, authorization, pluginId);
			await mapStoreErrors(() => runtime.store.deleteDraftVersion(pluginId, version));
		}
	}
