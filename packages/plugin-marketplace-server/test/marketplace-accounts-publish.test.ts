import { createHash, createPublicKey, generateKeyPairSync, type KeyObject, randomUUID, verify } from "node:crypto";
...
if (!TEST_DB_URL) {
	...
	describe("publishing", () => {
		it("publishes an uploaded plugin version end to end", async () => {
			...
			const listBefore = await request(app.getHttpServer())
				...
				.expect(200);
			expect(
				(listBefore.body as { plugins: Array<{ id: string }> }).plugins.some(({ id }) => id === PLUGIN_ID),
			).toBe(false);
				...
				.send(
					Buffer.from(
						zipSync({
							...
							"native/addon.node": new Uint8Array([0, 0, 0, 0]),
						}),
					),
				)
				.expect(400);
			expect(rejectedNativeName.body).toMatchObject({
				error: { code: "PAYLOAD_NATIVE_UNSUPPORTED" },
			});
			const rejectedNativeMagic = await request(app.getHttpServer())
				...
				.send(
					Buffer.from(
						zipSync({
							"index.ts": strToU8(ENTRY_SOURCE),
							"native/addon.dat": new Uint8Array([0x4d, 0x5a, 0, 0]),
						}),
					),
				)
				.expect(400);
			expect(rejectedNativeMagic.body).toMatchObject({
				error: { code: "PAYLOAD_NATIVE_UNSUPPORTED" },
			});
	...
	describe("persistence", () => {
		it("retains accounts, plugins, ratings, and stats across restarts", async () => {
			...
			const persistentConfig: MarketplaceServerConfig = { ...config, databaseUrl: dbUrl };
