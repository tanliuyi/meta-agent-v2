...
const HEX = /^[0-9a-f]{64}$/;
const NAME = /^[A-Za-z0-9][A-Za-z0-9._-]*\.(docx|txt)$/;
...
export function validateCorpusManifest(input: unknown): CorpusManifest {
	...
	const fixtures = root.fixtures.map((raw): CorpusEntry => {
		...
		const expectedKeys =
			expected.editableRun === undefined
				? ["inspect", "warnings", "operation"]
				: ["inspect", "editableRun", "warnings", "operation"];
...
export function loadCorpus(): { readonly root: string; readonly manifest: CorpusManifest } {
	const root = resolve(import.meta.dirname, "corpus");
